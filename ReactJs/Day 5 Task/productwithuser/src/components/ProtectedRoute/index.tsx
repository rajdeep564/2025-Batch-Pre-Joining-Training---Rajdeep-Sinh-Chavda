...
// Secret key (must match the one used in `Login.tsx`)
...
const verifyToken = async (token: string) => {
  try {
    ...
    return payload; // ✅ Returns decoded user details if valid
  } catch (error) {
...
const ProtectedRoute: React.FC = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      setIsAuthenticated(!!userData); // ✅ Authenticated if token is valid
    };
